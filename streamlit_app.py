...
st.info(
    """
Demo mode:
- Upload a small `candidates.jsonl` file for the fastest experience.
- The "Use Existing Dataset" option loads the full competition dataset (~100k candidates).
- Full-dataset reruns can take several minutes because embeddings are recomputed.
"""
)
...
st.caption("Use Existing Dataset loads the full ~100k competition dataset and will be slower.")
...
if use_existing:
    ...
    if st.session_state.uploaded_file_path != CANDIDATES_JSONL:

        st.session_state.uploaded_file_path = CANDIDATES_JSONL
    ...
    st.warning(
        "Using the full competition dataset (~100k candidates). This may take several minutes."
    )
